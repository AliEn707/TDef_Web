...
function mapEditor_saveTextures() {
	var text = "", usable = [], index = 0
	/////only usable
	for (var i = 0; i < nodesTextures.length; i++)
		if (typeof usable[nodesTextures[i]] === 'undefined') 
			usable[nodesTextures[i]] = index++
	for (var j = 0; j < 4; j++) 
		for (var i = 0; i < outerNodesTextures[j].length; i++) 
			if (typeof usable[outerNodesTextures[j][i]] === 'undefined')
				usable[outerNodesTextures[j][i]] = index++			
	if (walls.length > 0) 
		for (var i = 0; i < walls.length; i++)
			if (typeof usable[walls[i].texture] === 'undefined')
				usable[walls[i].texture] = index++			
	if (objects.length > 0)
		for (var i = 0; i < objects.length; i++)
			if (typeof usable[objects[i].texture] === 'undefined')
				usable[objects[i].texture] = index++
	/////
	var tmp = []
	for (var i in usable) {
	...
	text += '-\n'
	for (var i = 0; i < nodesTextures.length; i++) 
		text += (usable[nodesTextures[i]] + 1) + ' '
	text += '\n'
	for (var j = 0; j < 4; j++) {
		for (var i = 0; i < outerNodesTextures[j].length; i++) 
			text += (usable[outerNodesTextures[j][i]] + 1) + ' '
		text += '\n'
	}
	...
	if (walls.length > 0) {
		...
		for (var i = 0; i < walls.length; i++)
			text += walls[i].index + ' ' + walls[i].direction + ' ' + (usable[walls[i].texture] + 1) + '\n'
	}
	if (objects.length > 0) {
		text += 'objects ' + objects.length + '\n'
		for (var i = 0; i < objects.length; i++)
			text += Math.floor(objects[i].ind/size) + ' ' + objects[i].ind%size + ' ' + (usable[objects[i].texture] + 1) + '\n'
	}	
...
console.log(tempTextures)
	...
	while (length != big) {
		...
		for (var j = 0; j < nodes.length; j++)
			nodesTextures[length + j] = tempTextures[nodes[j] - 1]
		length += nodes.length
	}
	i++
	for (var j = 0; j < 4; j++) {
		...
		for (var k = 0; k < outerNodesTextures[j].length; k++) 
			outerNodesTextures[j][k] = tempTextures[nodes[k] - 1]
	}
	while (i < text.length) {
		if (text[i].search("minimap") != -1) { //load minimap
			...
			for (var j = 0; j < length; j++) {
				wall = text[i++].split(' ')
				walls.push({index: parseInt(wall[0]), direction: wall[1], texture: tempTextures[parseInt(wall[2]) - 1]})
			}	
		}
			...
			for (var j = 0; j < length; j++) {
				object = text[i++].split(' ') 
				objects.push({ind: parseInt(object[0])*size + parseInt(object[1]), texture: tempTextures[parseInt(object[2]) - 1]})
			}
		}
		else
			i++
