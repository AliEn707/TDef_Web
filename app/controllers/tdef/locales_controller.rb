class Tdef::LocalesController < ApplicationController
	...
	def get
    data=Rails.cache.fetch("tdef_locale/#{I18n.locale}",expires_in: 6.hours){"var locales=#{Tdef::Locale.where(name: I18n.locale).first.locale_datas.inject({}){|o,c| o.merge({c.key=>c.value})}.to_json};"}
		send_data(data, type: "text/javascript; charset=utf-8", filename: "locale_#{I18n.locale}.js", disposition:'inline')	
	end
end
